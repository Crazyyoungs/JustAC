...
Multi-agent audit of the SimC/AC priority ranking pipeline, triggered by the Balance Druid mis-ordering report.
16 finder agents (3 investigators + 13 per-class sweeps). Adversarial verification phase largely did NOT run
(hit the monthly spend limit after 58/318 agents). Treat spec-specific rank/id claims as single-pass — high
signal (16 independent finders converged on the same handful of mechanisms) but spot-check exact numbers before acting.
The Balance chain (Sunfire 93402 absent, moonfire 326646, kotg_st ordering) was independently confirmed by hand.
