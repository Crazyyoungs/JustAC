...
L["Max Icons desc"] = "显示的最大法术图标数量（1 = Blizzard 的建议，2+ = 后续队列）"
L["Icon Size desc"] = "法术图标的基本大小（像素）"
L["Spacing desc"] = "图标之间的间距（像素）"
L["Primary Spell Scale desc"] = "位置1图标（Blizzard 的建议）的缩放倍数"
...
L["Include All Available Abilities desc"] = "在 Blizzard 的建议（位置 1）中包含隐藏在宏条件（如 [mod:shift]）后面的法术。"
...
L["Blacklist Info"] = "从所有位置隐藏法术或物品：主队列、姓名板叠加层以及爆发/冲锋注入。\n\n|cffff6666Shift+右键点击|r任意队列图标可直接将其加入黑名单。（循环标签仅编辑位置 2+。）"
L["Blacklist Warning"] = "|cFFFF4444注意：|r 将位置 1（Blizzard 的建议）加入黑名单可能导致循环卡顿。关闭某个条目的 |cffFFAA00应用到位置 1|r，可仅从位置 2+ 隐藏它，让循环保持流畅。"
...
L["Blacklist Position 1 desc"] = "在位置 1（Blizzard 的建议）也隐藏此技能。|cffFFAA00关闭|r 则仅从位置 2+ 隐藏它。位置 1 仍会显示它，因此 Blizzard 的循环不会卡顿。"
-- Burst Injection
...
-- Custom Queue
L["Custom Queue"] = "循环"
L["Custom Queue Position Note"] = "|cffffd100位置 1 始终是 Blizzard 的实时建议|r，JustAC 无法更改它。本标签的所有设置都控制位置 2+：即显示为后续的技能队列。"
...
L["Custom Queue Context Aware desc"] = "重新排序位置 2+ 以适应当前形势，排在您保存的顺序之前：\n\n- 单体技能在面对单个敌人时上升；群体/顺劈技能在面对多个敌人时上升。\n- 当目标超出近战范围时，近战技能下沉。\n\n关闭：忽略形势，保持您的精确顺序。"
...
L["Enable Custom Queue desc"] = "启用后，您的自定义循环将设置位置 2+。禁用则使用 Blizzard 的循环。"
L["Custom Queue Spells"] = "自定义循环"
L["Custom Queue Spells desc"] = "您用于位置 2+ 的技能顺序。使用 上/下 重新排序；上方的排序选项仍然适用。"
