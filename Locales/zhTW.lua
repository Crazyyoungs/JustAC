...
L["Max Icons desc"] = "顯示的最大法術圖示數量（1 = 暴雪的選擇，2+ = 即將到來的佇列）"
L["Icon Size desc"] = "法術圖示的基本大小（像素）"
L["Spacing desc"] = "圖示之間的間距（像素）"
L["Primary Spell Scale desc"] = "位置1圖示（暴雪的選擇）的縮放倍數"
...
L["Include All Available Abilities desc"] = "在暴雪的選擇（位置1）中包含隱藏在巨集條件（如[mod:shift]）後面的法術。"
...
L["Blacklist Info"] = "從所有位置隱藏法術或物品：主佇列、名條覆蓋層，以及爆發/衝鋒注入。\n\n|cffff6666Shift+右鍵點擊|r任意佇列圖示即可直接將其加入黑名單。（循環標籤僅編輯位置 2+。）"
L["Blacklist Warning"] = "|cFFFF4444注意：|r 將位置 1（暴雪的選擇）加入黑名單可能導致循環卡頓。在條目上關閉 |cffFFAA00套用至位置1|r，即可僅從位置 2+ 隱藏它並保持循環順暢。"
...
L["Blacklist Position 1 desc"] = "也在位置1（暴雪的選擇）隱藏此技能。|cffFFAA00關閉|r則僅從位置 2+ 隱藏它。位置1 仍會顯示它，因此暴雪的循環不會卡頓。"
...
-- Custom Queue
L["Custom Queue"] = "循環"
L["Custom Queue Position Note"] = "|cffffd100位置 1 永遠是暴雪的即時選擇|r，JustAC 無法更改它。此標籤上的所有設定都控制位置 2+：顯示為即將到來的技能佇列。"
L["Custom Queue Info"] = "建立你自己的即將到來佇列，而不是鏡像暴雪的。在下方啟用，然後重新排序、刪減或新增技能。它一開始會以你專精的暴雪輔助戰鬥循環為基礎。\n\n要在所有位置隱藏某個技能，請|cffff6666Shift+右鍵點擊|r其佇列圖示。"
...
L["Custom Queue Context Aware desc"] = "重新排序位置 2+ 以配合當前情況，排在你儲存的順序之前：\n\n- 單體技能在面對單一敵人時上升；範圍/橫掃技能在面對多個敵人時上升。\n- 近戰技能在目標超出近戰範圍時下沉。\n\n關閉：忽略情況並保持你的確切順序。"
L["Custom Queue Sink Cooldowns"] = "冷卻技能殿後"
L["Custom Queue Sink Cooldowns desc"] = "將冷卻中的技能移到佇列末端。關閉：留在其列出的位置。"
...
L["Enable Custom Queue desc"] = "啟用後，你的自訂循環會設定位置 2+。停用則使用暴雪的循環。"
L["Custom Queue Spells"] = "自訂循環"
L["Custom Queue Spells desc"] = "你用於位置 2+ 的技能順序。使用上/下重新排序；上方的排序選項仍會套用。"
