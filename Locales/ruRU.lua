...
L["Primary Spell Scale"] = "Масштаб позиции 1"
...
L["Primary Only"] = "Только позиция 1"
...
L["Max Icons desc"] = "Максимум иконок (1 = выбор Blizzard, 2+ = предстоящая очередь)"
L["Icon Size desc"] = "Базовый размер иконок в пикселях"
L["Spacing desc"] = "Расстояние между иконками в пикселях"
L["Primary Spell Scale desc"] = "Множитель масштаба иконки позиции 1 (выбор Blizzard)"
...
L["Include All Available Abilities desc"] = "Включить заклинания, скрытые за условиями макросов вроде [mod:shift], в выбор Blizzard (позиция 1)."
...
L["Blacklist Info"] = "Скрывает заклинание или предмет со всех позиций: основная очередь, оверлей и инъекция рывка/сближения.\n\n|cffff6666Shift+Правый клик|r на любой иконке очереди, чтобы добавить её в чёрный список напрямую. (Вкладка «Ротация» редактирует только позиции 2+.)"
L["Blacklist Warning"] = "|cFFFF4444Внимание:|r блокировка позиции 1 (выбор Blizzard) может остановить ротацию. Отключите |cffFFAA00Применить к позиции 1|r у записи, чтобы скрыть её только с позиций 2+ и сохранить ход ротации."
...
L["Blacklist Position 1 desc"] = "Также скрывать эту способность на позиции 1 (выбор Blizzard). |cffFFAA00Отключите|r, чтобы скрыть её только с позиций 2+. Позиция 1 продолжит её показывать, поэтому ротация Blizzard не останавливается."
...
-- Custom Queue
L["Custom Queue"] = "Ротация"
L["Custom Queue Position Note"] = "|cffffd100Позиция 1 всегда занята живым выбором Blizzard|r, и JustAC не может его изменить. Всё на этой вкладке управляет позициями 2+: очередью способностей, показываемых как предстоящие."
L["Custom Queue Info"] = "Создайте свою предстоящую очередь вместо повторения выбора Blizzard. Включите её ниже, затем меняйте порядок, убирайте или добавляйте способности. Изначально она заполняется ротацией Assisted Combat для вашей специализации.\n\nЧтобы скрыть способность везде, |cffff6666Shift+Правый клик|r на её иконке в очереди."
...
L["Custom Queue Context Aware desc"] = "Менять порядок позиций 2+ под текущую ситуацию, перед вашим сохранённым порядком:\n\n- Способности по одной цели поднимаются против одного врага; AoE / по площади поднимаются против нескольких.\n- Способности ближнего боя опускаются, пока цель вне зоны ближнего боя.\n\nВыкл.: игнорировать ситуацию и сохранять ваш точный порядок."
L["Custom Queue Sink Cooldowns"] = "Перезарядки в конец"
L["Custom Queue Sink Cooldowns desc"] = "Перемещать способности на перезарядке в конец очереди. Выкл.: оставлять их на своих местах."
L["Custom Queue Blacklist Note"] = "Удаление здесь затрагивает только позиции 2+. Чтобы скрыть заклинание со всех позиций (включая 1), добавьте его в чёрный список: |cffff6666Shift+Правый клик|r на его иконке в очереди или используйте вкладку «Чёрный список»."
L["Enable Custom Queue"] = "Включить пользовательскую ротацию"
L["Enable Custom Queue desc"] = "При включении ваша пользовательская ротация задаёт позиции 2+. Отключите для использования ротации Blizzard."
L["Custom Queue Spells"] = "Пользовательская ротация"
L["Custom Queue Spells desc"] = "Ваш порядок способностей для позиций 2+. Используйте Вверх / Вниз для изменения порядка; параметры порядка выше всё равно применяются."
