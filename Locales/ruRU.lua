...
L["Blacklist Info"] = "Скрывает заклинание или предмет со всех позиций — основная очередь, оверлей табличек и инъекция рывка/сближения.\n\n|cffff6666Shift+Правый клик|r на любой иконке очереди, чтобы добавить её в чёрный список напрямую. (Вкладка «Фиксированная очередь» редактирует только позиции 2+.)"
...
-- Custom Queue
L["Custom Queue"] = "Фиксированная очередь"
L["Custom Queue Info"] = "Выберите, какие способности могут появляться на позициях 2+ (позиция 1 всегда выбирается Blizzard). Ваш порядок решает спорные случаи — JustAC по-прежнему выводит первыми проки и способности, подходящие к ситуации (AoE / одиночная цель, дальность), и опускает способности на перезарядке.\n\nЧтобы скрыть способность везде, добавьте её в чёрный список (|cffff6666Shift+Правый клик|r на её иконке в очереди)."
...
L["Enable Custom Queue"] = "Включить фиксированную очередь"
L["Enable Custom Queue desc"] = "При включении ваш список задаёт позиции 2+. Отключите для использования стандартной ротации Blizzard."
...
L["Custom Queue Stale Chat"] = "Фиксированная очередь может устареть —"
...
L["Refresh from Rotation desc"] = "Заменить всю фиксированную очередь текущим списком ротации Blizzard."
L["Refresh from Rotation confirm"] = "Ваша фиксированная очередь будет заменена текущей ротацией Blizzard. Ваш пользовательский порядок будет утерян. Продолжить?"
